...
async def ensure_coc_client() -> coc.Client:
    ...
    if shadow_reporter.enabled:
        raise RuntimeError("Clash API access disabled in shadow mode")
...
async def build_seeded_registrations_for_guild(guild_id: int) -> list[TeamRegistration]:
    ...
    if shadow_reporter.enabled:
        await shadow_reporter.report(
            None,
            f"[tournament] build_seeded_registrations skipped for {guild_id}",
        )
        return []
    if not COC_EMAIL or not COC_PASSWORD:
        raise RuntimeError(
...
async def setup_command(  # pragma: no cover - Discord slash command wiring
    ...
    if shadow_reporter.enabled:
        await interaction.response.send_message(
            "Tournament configuration recorded in shadow mode.",
            ephemeral=True,
        )
...
async def register_team_command(  # pragma: no cover - Discord slash command wiring
    ...
    if shadow_reporter.enabled:
        await interaction.followup.send(
            "Tournament registration recorded in shadow mode.",
            ephemeral=True,
        )
        ...
        await shadow_reporter.report(
            guild,
            "[tournament] simulated registration",
            embeds=[embed],
        )
        return
...
async def create_bracket_command(  # pragma: no cover - Discord slash command wiring
    ...
    if shadow_reporter.enabled:
        embed = build_bracket_embed(
            ...
            requested_by=interaction.user,
            summary_note=f"Simulated bracket with {len(registrations)} teams",
        )
        await send_ephemeral(
            interaction,
...
async def select_round_winner_command(  # pragma: no cover - Discord slash command wiring
    ...
    if shadow_reporter.enabled:
        await send_ephemeral(
            interaction,
            f"Winner selection for {match_identifier} recorded in shadow mode.",
        )
        embed = build_bracket_embed(
            bracket,
            title="Bracket Update",
            requested_by=interaction.user,
            summary_note=f"Simulated winner selection for {match_identifier}",
        )
        await shadow_reporter.report(
            guild,
            "[tournament] simulated winner selection",
            embeds=[embed],
        )
        return
...
async def simulate_tourney_command(  # pragma: no cover - Discord slash command wiring
    ...
    if shadow_reporter.enabled:
        embed = build_bracket_embed(
            ...
            requested_by=interaction.user,
            summary_note="Simulation executed in shadow mode",
        )
        await send_ephemeral(
            interaction,
...
async def on_ready() -> None:  # pragma: no cover - Discord lifecycle hook
    await tree.sync()
    if shadow_reporter.enabled:
        await shadow_reporter.report(None, "[tournament] on_ready shadow mode active")
        return
    ...
    try:
        async with bot:
            if not shadow_reporter.enabled:
                ...
                await bot.start(DISCORD_TOKEN)  # still start to observe events
    finally:
...
def configure_runtime(
    *,
    ...
    global bot, tree, dynamodb, table, storage, coc_client, shadow_reporter
    global TOURNAMENT_TABLE_NAME, _shadow_config
    ...
    if table_name is not None:
        TOURNAMENT_TABLE_NAME = table_name
        table = dynamodb.Table(TOURNAMENT_TABLE_NAME)
    elif table is None and TOURNAMENT_TABLE_NAME:
        table = dynamodb.Table(TOURNAMENT_TABLE_NAME)

    storage = TournamentStorage(table)

    if registration_channel_id is not None:
        global TOURNAMENT_REGISTRATION_CHANNEL_ID
        TOURNAMENT_REGISTRATION_CHANNEL_ID = registration_channel_id
